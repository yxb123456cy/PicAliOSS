...
const faqs = [
  ...
  {
    title: '图片列表加载缓慢？',
    content: '图片列表加载速度取决于您的网络以及OSS中的图片数量。插件已默认使用本地缓存以提升加载速度。如需查看最新图片，可点击列表页的“刷新”按钮。'
  }
];
...
const techStacks = [
  {
    ...
    description: '负责浏览器插件工程化、开发热更新、入口拆分与后续 Chrome / Firefox / Edge 的扩展适配基础。'
  },
  {
    emoji: '💚',
    title: 'Vue 3 + Composition API',
    description: '承载 Popup 界面与交互逻辑，适合拆分上传、管理、设置、帮助等功能视图，保证界面迭代效率。'
  },
  {
    emoji: '🔷',
    title: 'TypeScript',
    description: '为 OSS 配置、图片列表、工具函数和状态管理提供类型约束，降低插件长期维护时的回归风险。'
  },
  {
    emoji: '🎨',
    title: 'PrimeVue',
  ...
  },
  {
    ...
    description: '用于本地保存并加密 OSS 配置，尽量避免明文暴露敏感信息，兼顾插件端易用性与基础安全性。'
  }
];
</script>
    ...
    <div class="section">
      ...
      <Accordion :value="0">
        <AccordionPanel v-for="(faq, index) in faqs" :key="index" :value="index">
          ...
          <AccordionContent>
            <p style="white-space: pre-wrap; font-size: 14px; color: #475569; margin: 0;">{{ faq.content }}</p>
          </AccordionContent>
        </AccordionPanel>
      </Accordion>
    </div>
